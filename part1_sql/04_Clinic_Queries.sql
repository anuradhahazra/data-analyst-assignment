...
WITH rev AS (
    SELECT DATE_FORMAT(datetime, '%Y-%m') AS year_month, SUM(amount) AS revenue
    FROM clinic_sales
    WHERE datetime >= '2021-01-01' AND datetime < '2022-01-01'
    GROUP BY DATE_FORMAT(datetime, '%Y-%m')
),
exp AS (
    SELECT DATE_FORMAT(datetime, '%Y-%m') AS year_month, SUM(amount) AS expense
    FROM expenses
    WHERE datetime >= '2021-01-01' AND datetime < '2022-01-01'
    GROUP BY DATE_FORMAT(datetime, '%Y-%m')
)
SELECT COALESCE(r.year_month, e.year_month) AS year_month,
...
SELECT cp.city, cp.cid, cp.clinic_name, cp.profit
FROM (
...
WITH clinic_profit AS (
    SELECT c.cid, c.clinic_name, c.state,
           COALESCE((SELECT SUM(amount) FROM clinic_sales cs WHERE cs.cid = c.cid AND DATE_FORMAT(cs.datetime,'%Y-%m')='2021-09'),0) AS revenue,
           COALESCE((SELECT SUM(amount) FROM expenses ex WHERE ex.cid = c.cid AND DATE_FORMAT(ex.datetime,'%Y-%m')='2021-09'),0) AS expense
    FROM clinics c
),
profit_calc AS (
    SELECT cid, clinic_name, state, (revenue - expense) AS profit
    FROM clinic_profit
)
